  ...
  # NHL Team Information with simple text logos (no problematic
  emojis)
  NHL_TEAMS = {
      'ANA': {'name': 'Anaheim Ducks', 'logo': 'ANA', 'division':
  'Pacific'},
      'BOS': {'name': 'Boston Bruins', 'logo': 'BOS', 'division':
  'Atlantic'},
      'BUF': {'name': 'Buffalo Sabres', 'logo': 'BUF', 'division':
  'Atlantic'},
      'CGY': {'name': 'Calgary Flames', 'logo': 'CGY', 'division':
  'Pacific'},
      'CAR': {'name': 'Carolina Hurricanes', 'logo': 'CAR',
  'division': 'Metropolitan'},
      'CHI': {'name': 'Chicago Blackhawks', 'logo': 'CHI', 'division':
   'Central'},
      'COL': {'name': 'Colorado Avalanche', 'logo': 'COL', 'division':
   'Central'},
      'CBJ': {'name': 'Columbus Blue Jackets', 'logo': 'CBJ',
  'division': 'Metropolitan'},
      'DAL': {'name': 'Dallas Stars', 'logo': 'DAL', 'division':
  'Central'},
      'DET': {'name': 'Detroit Red Wings', 'logo': 'DET', 'division':
  'Atlantic'},
      'EDM': {'name': 'Edmonton Oilers', 'logo': 'EDM', 'division':
  'Pacific'},
      'FLA': {'name': 'Florida Panthers', 'logo': 'FLA', 'division':
  'Atlantic'},
      'LAK': {'name': 'Los Angeles Kings', 'logo': 'LAK', 'division':
  'Pacific'},
      'MIN': {'name': 'Minnesota Wild', 'logo': 'MIN', 'division':
  'Central'},
      'MTL': {'name': 'Montreal Canadiens', 'logo': 'MTL', 'division':
   'Atlantic'},
      'NSH': {'name': 'Nashville Predators', 'logo': 'NSH',
  'division': 'Central'},
      'NJD': {'name': 'New Jersey Devils', 'logo': 'NJD', 'division':
  'Metropolitan'},
      'NYI': {'name': 'New York Islanders', 'logo': 'NYI', 'division':
   'Metropolitan'},
      'NYR': {'name': 'New York Rangers', 'logo': 'NYR', 'division':
  'Metropolitan'},
      'OTT': {'name': 'Ottawa Senators', 'logo': 'OTT', 'division':
  'Atlantic'},
      'PHI': {'name': 'Philadelphia Flyers', 'logo': 'PHI',
  'division': 'Metropolitan'},
      'PIT': {'name': 'Pittsburgh Penguins', 'logo': 'PIT',
  'division': 'Metropolitan'},
      'SJS': {'name': 'San Jose Sharks', 'logo': 'SJS', 'division':
  'Pacific'},
      'SEA': {'name': 'Seattle Kraken', 'logo': 'SEA', 'division':
  'Pacific'},
      'STL': {'name': 'St. Louis Blues', 'logo': 'STL', 'division':
  'Central'},
      'TBL': {'name': 'Tampa Bay Lightning', 'logo': 'TBL',
  'division': 'Atlantic'},
      'TOR': {'name': 'Toronto Maple Leafs', 'logo': 'TOR',
  'division': 'Atlantic'},
      'UTA': {'name': 'Utah Hockey Club', 'logo': 'UTA', 'division':
  'Central'},
      'VAN': {'name': 'Vancouver Canucks', 'logo': 'VAN', 'division':
  'Pacific'},
      'VGK': {'name': 'Vegas Golden Knights', 'logo': 'VGK',
  'division': 'Pacific'},
      'WSH': {'name': 'Washington Capitals', 'logo': 'WSH',
  'division': 'Metropolitan'},
      'WPG': {'name': 'Winnipeg Jets', 'logo': 'WPG', 'division':
  'Central'}
  }

  # Team name mapping (full names to codes)
  TEAM_NAME_TO_CODE = {
      'Anaheim Ducks': 'ANA',
      'Boston Bruins': 'BOS',
      'Buffalo Sabres': 'BUF',
  ...
  def sort_injuries_by_date(injuries):
      """Sort injuries in reverse chronological order (newest
  first)"""
      def get_sort_key(injury):
          injury_date = injury.get('injury_date', '')
          scraped_at = injury.get('scraped_at', '')

          # Try to parse injury_date first, then scraped_at, then
  current time
  ...
  def create_team_grid_with_injuries():
      """Create compact team grid with all teams visible on one
  screen"""
      grid_data = {}
  ...
  team_name.upper())
          print(f"Processing injury: {injury.get('player')} - Team:
  '{team_name}' -> Code: '{team_code}'")
      ...
      for team_code in grid_data:
          if grid_data[team_code]['injuries']:
              grid_data[team_code]['injuries'] =
  sort_injuries_by_date(grid_data[team_code]['injuries'])
      ...
      # Sort all teams alphabetically by name
  ...
  %H:%M:%S')
          ...
          # Ensure team grid is available even if scraping fails
  ...
  def api_refresh():
      ...
      update_injury_data()
      return jsonify({'status': 'updated', 'timestamp':
  injury_data['last_updated']})
