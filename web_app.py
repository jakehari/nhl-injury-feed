...
def parse_injury_date(date_string):
    """Parse injury date from various formats"""
    if not date_string:
        return None
    
    try:
        # Handle "Fri, Sep 12, 2025" format
        if ',' in date_string and len(date_string.split(',')) >= 2:
            date_part = date_string.split(',')[1].strip() + ', ' + date_string.split(',')[2].strip()
...
def extract_return_timeline(injury_type_text):
    """Extract expected return timeline from injury description"""
    if not injury_type_text:
        return None
    
    text = injury_type_text.lower()
    
    # Common return timeline patterns
    timeline_patterns = [
        ...
        ('month to month', 'Month-to-month'),
        ('out indefinitely', 'Out indefinitely'),
        ('indefinitely', 'Out indefinitely'),
        ('season-ending', 'Season-ending'),
        ('season ending', 'Season-ending'),
        ('out for season', 'Season-ending'),
        ('out for the season', 'Season-ending'),
        ('ltir', 'LTIR'),
        ('long-term', 'Long-term IR'),
        ('ir-eligible', 'IR-eligible'),
        ('expected back', 'Return expected'),
    ...
    # Look for specific week/month timelines
    import re
    
    # Match patterns like "2-3 weeks", "4-6 weeks", "out 2 weeks"
    week_pattern = r'(?:out\s+)?(\d+)(?:-(\d+))?\s+weeks?'
    week_match = re.search(week_pattern, text)
    if week_match:
        if week_match.group(2):
            return f"{week_match.group(1)}-{week_match.group(2)} weeks"
        else:
            return f"{week_match.group(1)} weeks"
    
    # Match patterns like "2-3 months", "1 month"
    month_pattern = r'(?:out\s+)?(\d+)(?:-(\d+))?\s+months?'
    month_match = re.search(month_pattern, text)
    if month_match:
        if month_match.group(2):
            return f"{month_match.group(1)}-{month_match.group(2)} months"
        else:
            return f"{month_match.group(1)} months"
    
    # Match patterns like "miss 5 games", "out 10 games"
    games_pattern = r'(?:miss|out)\s+(\d+)\s+games?'
    games_match = re.search(games_pattern, text)
    if games_match:
        return f"Miss {games_match.group(1)} games"
                                ...
                                if team_stats:
                                    ...
                                    for player in all_players:
                                        ...
                                        if ':' in ice_time_str:
                                            ...
                                            min_threshold = 12 if position == 'F' else 15
                                            ...
                                            if 0 < ice_time_minutes < min_threshold:
                                                possible_injury = {
                                                    ...
                                                    'injury_date': latest_game.get('gameDate'),
                                                    'reason': f'Low ice time ({ice_time_str}) for {position}',
                                                    'return_timeline': 'Unknown',
                                                    'confidence': 'Medium',
                                                    'type': 'possible',
...
def sort_injuries_by_date(injuries):
    """Sort injuries in reverse chronological order (most recent first)"""
    def get_sort_key(injury):
        # Try to parse from various date fields
        for date_field in ['injury_date', 'injury_type', 'scraped_at', 'game_date']:
            date_str = injury.get(date_field, '')
            if date_str:
                parsed_date = parse_injury_date(date_str)
                if parsed_date:
                    return parsed_date
...
def enhance_injury_data(injuries):
    """Enhance injury data with return timeline and proper date parsing"""
    enhanced_injuries = []
    
    for injury in injuries:
        enhanced_injury = injury.copy()
        
        # Extract return timeline from injury type
        injury_type = injury.get('injury_type', '')
        return_timeline = extract_return_timeline(injury_type)
        enhanced_injury['return_timeline'] = return_timeline
        
        # Parse and format injury date
        injury_date = injury.get('injury_date', '') or injury.get('injury_type', '')
        parsed_date = parse_injury_date(injury_date)
        if parsed_date:
            enhanced_injury['parsed_date'] = parsed_date
            enhanced_injury['formatted_date'] = parsed_date.strftime('%b %d, %Y')
        else:
            enhanced_injury['formatted_date'] = 'Unknown date'
        
        enhanced_injuries.append(enhanced_injury)
...
def create_team_grid_with_injuries():
    ...
    # Sort injuries within each team by date (most recent first)
...
def update_injury_data():
    ...
    try:
        ...
        raw_injuries = scrape_hockey_reference_injuries()
        
        # Enhance injury data with return timelines and proper dates
        injuries = enhance_injury_data(raw_injuries)
        ...
        possible_injuries = enhance_injury_data(possible_injuries)
        ...
        # Sort injuries by date within each team
        for team in by_team:
